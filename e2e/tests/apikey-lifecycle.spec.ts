...
test.describe('API Key Lifecycle', () => {
  ...
  test('should complete full API key lifecycle: request, reveal, and delete', async ({ page }) => {
    ...
    const apiProductSelect = page.locator('#api-product-typeahead-select');
    await apiProductSelect.click();
    await page.waitForTimeout(500);

    // Type to filter for payment-api
    const apiProductSearch = page.locator('#api-product-typeahead-select input');
    ...
    const tierSelect = page.locator('#tier-typeahead-select');
    await expect(tierSelect).toBeEnabled({ timeout: 10000 });
    ...
    // Submit the request
    const submitButton = page.locator('button:has-text("Request")');
  ...
  test('should show warning when no API products are available', async ({ page }) => {
    // This test would need a namespace with no API products
    // For now, we'll skip it or use a different namespace if available
